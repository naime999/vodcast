                ...
                <div class="card-body table-responsive">
                    <form class="row contentStore" action="{{ route('users.content.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="col-md-6 mb-2 form-group">
                            ...
                            <div class="video-view rounded w-50 overflow-hidden"></div>
                            @error('youtube_id')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        $(document).on("input", "#youtube-id", function () {
            const videoId = $(this).val().trim(); // Get the input value
            const embedUrl = `https://www.youtube.com/embed/${videoId}`;
            const thumbnailUrl = `https://img.youtube.com/vi/${videoId}/hqdefault.jpg`;

            // Validate the YouTube video ID (basic check for 11-character ID)
            if (videoId.length === 11) {
                ...
                // Clear the video view if the ID is invalid or empty
